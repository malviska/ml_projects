...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    # Your code here
    result =  label*(np.dot(feature_vector,theta) + theta_0)
    if result >= 1:
      return 0
    return 1-result
...
def perceptron_single_step_update(
    ...
    """
    # Your code here
    loss = label*(np.dot(feature_vector,current_theta) + current_theta_0)
    if loss <= 0:
...
def perceptron(feature_matrix, labels, T):
    ...
    """
    # Your code here
    (nsamples, ndim)  = feature_matrix.shape
    theta = np.zeros(ndim)
    theta_0 = 0
    for t in range(T):
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
      for i in get_order(nsamples):
          ...
          sum_theta += theta
          sum_theta_0 += theta_0
    sum_theta /= nsamples*T
    sum_theta_0 /= nsamples*T
    return (sum_theta, sum_theta_0)
...
def pegasos_single_step_update(
    ...
    if loss <= 1:
      theta = theta*(1-eta*L) + eta*label*feature_vector
...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    # Your code here
    result = np.dot(feature_matrix, theta) + theta_0
    arr =  np.where(result > 0, 1, -1) 
    return arr
...
def classifier_accuracy(
    ...
    result_train = classify(train_feature_matrix,theta, theta_0)
    result_test = classify(val_feature_matrix,theta, theta_0)
    acc_train = accuracy(result_train,train_labels)
    acc_test = accuracy(result_test,val_labels)
